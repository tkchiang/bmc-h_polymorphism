...
| | | |
| `SAXS_MCMC_analysis/` | `run_SAXS_MCMC.ipynb` | Implements MCMC on processed SAXS data and generates backend files for each experiment. Depends on the functions defined in `saxs_helpers.py`. Outputs `emcee` backend files with names like `backend_{datafile}.h5`. |
