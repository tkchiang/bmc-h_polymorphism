...
`SAXS_MCMC/` contains:
  - `SAXS_MCMC.ipynb` is a Jupyter notebook that implements MCMC on processed small-angle x-ray scattering data and generates backend files for each experiment.
...
`cgMD_curvature_analysis/` contains:
  - `cgMD_trajectory_analysis.ipynb` is a Jupyter notebook that reads cgMD trajectory data of fluctuating BMC-H hexamer lattice sheets and computes the local curvature of the sheets over time.
