...
| Folder | Main Notebooks / Scripts | Description / Output |
|--------|-------------------------|--------------------|
| `Nanotube_TEM_analysis/` | `nanotube_TEM_analysis.ipynb` | Automates analysis of nanotube TEM images. Outputs `results.csv` with diameters and chiral angles. |
| | `plot_results.ipynb` | Plots the data in `results.csv`. |
| `SAXS_MCMC/` | `run_SAXS_MCMC.ipynb` | Implements MCMC on processed SAXS data and generates backend files for each experiment. |
| | `backend_analysis.ipynb` | Analyzes the `emcee` backend files generated by `run_SAXS_MCMC.ipynb`. |
| | `saxs_models.py` | Contains functions for plotting and defining SAXS models used in fitting. |
| | `5djb_FoXS.dat` | Simulated scattering curve of a BMC-H subunit (PDB ID: 5DJB), interpolated to match experimental data. |
